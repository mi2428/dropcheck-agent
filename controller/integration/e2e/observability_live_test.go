...
func (cfg *e2eConfig) configureStandaloneUploadFesta(t *testing.T, festa string, uploadURL string) {
	...
	commands := []string{
		...
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt match essid %s", festa, quoteToken(ssid)),
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt credential passphrase %s", festa, quoteToken(psk)),
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt security auto", festa),
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt band all", festa),
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt wait ip", festa),
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt wait validated", festa),
		fmt.Sprintf("config> set standalone festa %s wifi-group mgmt timeout 25000", festa),
		fmt.Sprintf("config> set standalone festa %s check dns name %s type A timeout 8000", festa, standaloneDNSName),
		fmt.Sprintf("config> set standalone festa %s check ping host %s count 1 timeout 8000", festa, standalonePingHost),
		fmt.Sprintf("config> set standalone festa %s check http url %s expected-status 204 timeout 10000", festa, standaloneHTTPURL),
