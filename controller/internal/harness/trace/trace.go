// Package trace provides Dropcheck Harness expectations for traceroute checks.
package trace
...
func MaxHops() harness.OrderedMetric[uint32] {
...
func ExitCode() harness.OrderedMetric[int32] {
	return harness.Ordered[int32]("trace.exit_code", func(result harness.Result) (int32, bool, string) {
		trace, ok, reason := from(result)
...
func Elapsed() harness.OrderedMetric[time.Duration] {
	return harness.Ordered[time.Duration]("trace.elapsed", func(result harness.Result) (time.Duration, bool, string) {
		trace, ok, reason := from(result)
...
func OutputContains(value string) harness.Expectation {
...
func (e outputContains) Evaluate(result harness.Result) []harness.Finding {
	trace, ok, reason := from(result)
	...
	return []harness.Finding{harness.Fail(metric, "<not found>", "contains "+e.value, "output did not contain value")}
}
