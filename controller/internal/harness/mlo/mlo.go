// Package mlo provides Dropcheck Harness expectations for Wi-Fi 7 MLO diagnostics.
package mlo
...
func (ConnectedSelector) Present() harness.Expectation {
...
func (ScanSelector) CandidateCount() harness.OrderedMetric[int] {
	return harness.Ordered[int]("mlo.scan.candidate_count", func(result harness.Result) (int, bool, string) {
		mlo, ok, reason := from(result)
...
func (s GroupSelector) Exists() harness.Expectation {
	return groupExists{selector: s}
}

// Count matches the number of selected MLO scan groups.
func (s GroupSelector) Count() harness.OrderedMetric[int] {
	return harness.Ordered[int]("mlo.scan.group_count", func(result harness.Result) (int, bool, string) {
		mlo, ok, reason := from(result)
		if !ok {
...
func (s GroupSelector) LinkCount() harness.OrderedMetric[int] {
	return harness.Ordered[int]("mlo.scan.group_link_count", func(result harness.Result) (int, bool, string) {
		mlo, ok, reason := from(result)
		if !ok {
			return 0, false, reason
		}
...
func (e groupExists) Evaluate(result harness.Result) []harness.Finding {
	...
	if len(matches) > 0 {
		return []harness.Finding{harness.Pass("mlo.scan.group", describeGroup(matches[0]), "exists")}
	}
	return []harness.Finding{harness.Fail("mlo.scan.group", describeGroups(mlo.Groups), "exists", "no mlo scan group matched selector")}
}
...
func (CurrentRelationSelector) ConnectedMLDSeenInScan() harness.BoolMetric {
	return harness.Bool("mlo.relation.connected_mld_seen_in_scan", func(result harness.Result) (bool, bool, string) {
		mlo, ok, reason := from(result)
		if !ok {
...
func (CurrentRelationSelector) AssociatedLinksCoveredByScan() harness.BoolMetric {
...
func (MetadataSelector) Complete() harness.BoolMetric {
	return harness.Bool("mlo.metadata.complete", func(result harness.Result) (bool, bool, string) {
		mlo, ok, reason := from(result)
		if !ok {
			return false, false, reason
		}
...
func (f StringField) Known() harness.Expectation {
...
func (f StringField) Eq(value string) harness.Expectation {
...
func (e stringExpectation) Evaluate(result harness.Result) []harness.Finding {
	...
	return []harness.Finding{harness.Fail(e.field.metric, value, e.op, "constraint failed")}
}
...
func (e intExpectation) Evaluate(result harness.Result) []harness.Finding {
	...
	return []harness.Finding{harness.Fail(e.field.metric, fmt.Sprint(value), e.op, "constraint failed")}
}
...
func (e boolExpectation) Evaluate(result harness.Result) []harness.Finding {
	...
	return []harness.Finding{harness.Fail(e.metric, fmt.Sprintf("%t", value), fmt.Sprintf("== %t", e.expected), "constraint failed")}
}
