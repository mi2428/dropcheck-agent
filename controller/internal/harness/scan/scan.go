// Package scan provides Dropcheck Harness expectations for Wi-Fi scan checks.
package scan
...
func ResultCount() harness.OrderedMetric[int] {
	return harness.Ordered[int]("scan.result_count", func(result harness.Result) (int, bool, string) {
...
func ErrorCount() harness.OrderedMetric[int] {
	return harness.Ordered[int]("scan.error_count", func(result harness.Result) (int, bool, string) {
		scan, ok, reason := from(result)
...
func (s APSelector) Exists() harness.Expectation {
	return apExists{selector: s}
}

// Count matches the number of scan results selected by the selector.
func (s APSelector) Count() harness.OrderedMetric[int] {
...
func (e apExists) Evaluate(result harness.Result) []harness.Finding {
	...
	if len(matches) > 0 {
		return []harness.Finding{harness.Pass(e.selector.metric, describeAP(matches[0]), "exists")}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, describeAPs(scan.Results), "exists", "no scan result matched selector")}
}
