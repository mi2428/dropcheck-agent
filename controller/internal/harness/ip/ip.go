// Package ip provides Dropcheck Harness expectations for IP status checks.
package ip
...
func (s AddressSelector) InCIDR(cidr string) harness.Expectation {
	return addressInCIDR{selector: s, cidr: cidr}
}

// Scope requires at least one selected address to have scope.
func (s AddressSelector) Scope(scope string) harness.Expectation {
...
func (e addressScope) Evaluate(result harness.Result) []harness.Finding {
	...
	for _, address := range ip.Addresses {
		...
		if address.Scope == e.scope {
			return []harness.Finding{harness.Pass(e.selector.metric, address.Raw, "scope "+e.scope)}
		}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(ip.AddressStrings, ","), "scope "+e.scope, "no selected address had scope")}
}
...
func (e addressInCIDR) Evaluate(result harness.Result) []harness.Finding {
	...
	if err != nil {
		return []harness.Finding{harness.Fail(e.selector.metric, "<invalid cidr>", "valid CIDR", err.Error())}
	}
	for _, address := range ip.Addresses {
		if !e.selector.accepts(address.Addr) {
			continue
		...
		if prefix.Contains(address.Addr) {
			return []harness.Finding{harness.Pass(e.selector.metric, address.Raw, "in "+e.cidr)}
		}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(ip.AddressStrings, ","), "in "+e.cidr, "no selected address was inside CIDR")}
}
...
func (s DNSAddressSelector) InCIDR(cidr string) harness.Expectation {
...
func (e dnsAddressInCIDR) Evaluate(result harness.Result) []harness.Finding {
	...
	for _, address := range ip.DNSAddresses {
		...
		if prefix.Contains(address) {
			return []harness.Finding{harness.Pass(e.selector.metric, address.String(), "in "+e.cidr)}
		}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(ip.DNSServers, ","), "in "+e.cidr, "no selected DNS server was inside CIDR")}
}
...
func (s PrefixSelector) Within(cidr string) harness.Expectation {
...
func (e prefixWithin) Evaluate(result harness.Result) []harness.Finding {
	...
	for _, address := range ip.Addresses {
		...
		if prefixContainsPrefix(expected, address.Prefix) {
			return []harness.Finding{harness.Pass(e.selector.metric, address.Prefix.String(), "within "+e.cidr)}
		}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(ip.AddressStrings, ","), "within "+e.cidr, "no selected assigned prefix was contained by CIDR")}
}
...
func (s RouteSelector) Exists() harness.Expectation {
	return routeExists{selector: s}
}

// Count matches the number of parsed routes selected by the selector.
func (s RouteSelector) Count() harness.OrderedMetric[int] {
	return harness.Ordered[int](s.metric+"_count", func(result harness.Result) (int, bool, string) {
		if s.err != nil {
...
func (e routeExists) Evaluate(result harness.Result) []harness.Finding {
	...
	if len(matches) > 0 {
		return []harness.Finding{harness.Pass(e.selector.metric, matches[0].Raw, "exists")}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(ip.Routes, ","), "exists", "no parsed route matched selector")}
}
...
func (s StringListSelector) Contains(value string) harness.Expectation {
	return stringListContains{selector: s, value: value}
}

// ContainsPrefix requires one selected value to have prefix.
func (s StringListSelector) ContainsPrefix(prefix string) harness.Expectation {
...
func (e stringListContains) Evaluate(result harness.Result) []harness.Finding {
	...
	if slices.Contains(values, e.value) {
		return []harness.Finding{harness.Pass(e.selector.metric, e.value, "contains "+e.value)}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(values, ","), "contains "+e.value, "value not found")}
}
...
func (e stringListPrefix) Evaluate(result harness.Result) []harness.Finding {
	...
	for _, value := range values {
		if strings.HasPrefix(value, e.prefix) {
			return []harness.Finding{harness.Pass(e.selector.metric, value, "contains prefix "+e.prefix)}
		}
	}
	return []harness.Finding{harness.Fail(e.selector.metric, strings.Join(values, ","), "contains prefix "+e.prefix, "prefix not found")}
}
...
func (s TextSelector) Contains(value string) harness.Expectation {
...
func (e textContains) Evaluate(result harness.Result) []harness.Finding {
	...
	return []harness.Finding{harness.Fail(e.selector.metric, value, "contains "+e.value, "value not found")}
}
