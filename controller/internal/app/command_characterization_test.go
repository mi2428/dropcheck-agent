...
func TestLinuxWifiOperationalCommands(t *testing.T) {
	tests := []struct {
		...
		{
			name: "wifi wait",
			args: []string{"request", "wifi", "wait", "connected", "Lab", "--security", "transition", "--band", "5ghz", "--ip", "--validated", "--timeout", "9000"},
		...
		{
			name: "wifi assert",
			args: []string{"request", "wifi", "assert", "--ssid", "Lab", "--bssid", "aa:bb:cc:dd:ee:ff", "--security", "wpa3", "--band", "6ghz", "--ip", "--validated", "--timeout", "8000"},
			assert: func(t *testing.T, cmd *controlpb.RunCommand) {
		...
		{
			name: "wifi monitor interval option gets default duration",
			args: []string{"request", "monitor", "wifi", "--interval", "250"},
			assert: func(t *testing.T, cmd *controlpb.RunCommand) {
				if got := cmd.GetMonitorWifi(); got == nil || got.GetDurationMs() != 10000 || got.GetIntervalMs() != 250 {
		...
		},
		{
			...
			args: []string{"request", "monitor", "wifi", "--duration", "5000", "--interval", "250"},
			assert: func(t *testing.T, cmd *controlpb.RunCommand) {
		...
		},
		{
			...
			args: []string{"request", "wifi", "reconnect", "--timeout", "9000"},
			assert: func(t *testing.T, cmd *controlpb.RunCommand) {
