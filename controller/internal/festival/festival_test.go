...
func TestRunEvaluatesStandaloneArchiveResults(t *testing.T) {
	...
	festival.Run(t, festival.Plan{
		Results: []festival.ResultSource{
			festival.StandaloneArchive("standalone-smoke", archive),
		},
		Checks: []festival.Check{
			festival.Ping("8.8.8.8").
				Count(3).
			...
			festival.DNS("example.com").
				A().
				Expect(dns.AnswerCount().Ge(1), dns.Elapsed().Le(time.Second)),
			festival.HTTP("http://example.com/health").
				ExpectedStatus(204).
				Expect(festival.Assert("http matched", func(r festival.Result) error {
					if !r.Run.Raw.GetHttpCheck().GetMatched() {
						return fmt.Errorf("http check did not match")
...
func TestStandaloneArchiveResultSourcesLoadProtobufBinary(t *testing.T) {
	...
	targets, err := festival.StandaloneArchiveBytes("bytes", data).Targets()
	if err != nil {
	...
	targets, err = festival.StandaloneArchiveFile(path).Targets()
	if err != nil {
