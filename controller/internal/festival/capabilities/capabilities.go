...
func ErrorCount() festival.OrderedMetric[int] {
	return festival.Ordered[int]("capabilities.error_count", func(result festival.Result) (int, bool, string) {
...
func (e supportExpectation) Evaluate(result festival.Result) []festival.Finding {
	capabilities, ok, reason := from(result)
	if !ok {
		return []festival.Finding{festival.Fail(e.selector.metric, "<missing>", e.expected(), reason)}
	}
	if e.wantSupported {
		if slices.Contains(e.selector.supported(capabilities), e.selector.value) {
			return []festival.Finding{festival.Pass(e.selector.metric, e.selector.value, e.expected())}
		}
		return []festival.Finding{festival.Fail(e.selector.metric, describeSupport(e.selector.supported(capabilities), e.selector.unsupported(capabilities)), e.expected(), "capability is not supported")}
	}
	if slices.Contains(e.selector.unsupported(capabilities), e.selector.value) {
		return []festival.Finding{festival.Pass(e.selector.metric, e.selector.value, e.expected())}
	}
	return []festival.Finding{festival.Fail(e.selector.metric, describeSupport(e.selector.supported(capabilities), e.selector.unsupported(capabilities)), e.expected(), "capability is not unsupported")}
}
...
func (s FieldSelector) Eq(value string) festival.Expectation {
...
func (s FieldSelector) Contains(value string) festival.Expectation {
...
func (e fieldExpectation) Evaluate(result festival.Result) []festival.Finding {
	capabilities, ok, reason := from(result)
	metric := "capabilities.field." + e.selector.key
	if !ok {
		return []festival.Finding{festival.Fail(metric, "<missing>", e.op+" "+e.value, reason)}
	}
	for _, field := range capabilities.Fields {
		...
		if e.pass(field.GetValue()) {
			return []festival.Finding{festival.Pass(metric, field.GetValue(), e.op+" "+e.value)}
		}
		return []festival.Finding{festival.Fail(metric, field.GetValue(), e.op+" "+e.value, "field constraint failed")}
	}
	return []festival.Finding{festival.Fail(metric, "<missing>", e.op+" "+e.value, "field not found")}
}
